<?php
	...
	class TypesModel
	{
		...
		public static function get($id)
		{
			try
			{
				$command = "SELECT * FROM " . self::TABLE_NAME . " WHERE " . self::IDTYPE."=?";
				$statement = DBConnection::getInstance()->obtainDB()->prepare($command);
		...
		public static function create($data)
		{
			...
			try
			{
				...
				$command = "INSERT INTO " . self::TABLE_NAME . " ( " . self::IDTYPE . ", " . self::NAME . " ) VALUES (?, ?)";
				$statement = DBConnection::getInstance()->obtainDB()->prepare($command);
		...
		public static function delete($id)
		{
			try
			{
				$command = "DELETE FROM " . self::TABLE_NAME . " WHERE " . self::IDTYPE. "=?";
				$statement = DBConnection::getInstance()->obtainDB()->prepare($command);
		...
		public static function update($id, $data)
		{
			try
			{
				$idtype = $data->idtype;
				$name = $data->name;
				$command = "UPDATE " . self::TABLE_NAME . " SET " . self::IDTYPE. " =?, " .self::NAME. " =?   WHERE " . self::IDTYPE . "=?"; 
				$statement = DBConnection::getInstance()->obtainDB()->prepare($command);
